...
datadir2 = '/fmri/stats_spm12/EM/stats_spm8_swcar/';
[~,subid,~] = xlsread('F:/EM/2nd_spm_code/example_data.csv',1,'A:A');  
[CAR,~,~]   = xlsread('F:/EM/2nd_spm_code/example_data.csv',1,'D:D');
[SEX,~,~]   = xlsread('F:/EM/2nd_spm_code/example_data.csv',1,'B:B');
[AGE,~,~]   = xlsread('F:/EM/2nd_spm_code/example_data.csv',1,'C:C');
subid=subid'
datadir = 'F:/EM_brain/datacon000/activation/emovscon/'% the path keeps all con files
cd(datadir)
for i = 1:50                    
    yearfolder = ['20' subid{i}(1:2)];
    c1{i,1} = fullfile(datadir1, yearfolder, subid{i}, datadir2, ['con_0003','_',subid{i},'.nii',1]);   % rs
end

c1{i,1} = fullfile(datadir,['con_0003_*',subid,'.nii',1]); 

% -------------------GroupStats-------------------
spm_jobman('initcfg');
clear matlabbatch;
%variates
matlabbatch{1}.spm.stats.factorial_design.dir = {'F:/EM_brain/result/activation/'};
